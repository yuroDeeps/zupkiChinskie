...
public class ZupkaController {
    ...
    public ResponseEntity<Zupka> createZupka(@RequestBody Zupka zupka) {
        ...
        try {
            if (zupka.getMarka().isEmpty() || zupka.getMarka().isBlank() || zupka.getMarka() == "") {
                System.out.println("Bład działa");
            ...
            if (zupka.getPochodzenie().isEmpty() || zupka.getPochodzenie().isBlank() || zupka.getPochodzenie() == "") {
                return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
            }
            if (zupka.getWaga()<=0) {
                return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
